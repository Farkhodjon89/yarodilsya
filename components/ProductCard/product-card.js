          ...
          <Box sx={{
            ...
            <Typography
                sx={{
                  ...
                  fontWeight: '400',
                  marginBottom: '30px'
                }}>Артикул: {product.sku}</Typography>
          ...
          <Grid item sx={12} md={6}>
            <Box sx={{marginBottom: '40px'}}>
              ...
              <Typography sx={{
                ...
                "-webkit-box-orient": !open ? 'vertical' : '',
                "-webkit-line-clamp": !open ? '3' : '',
                textOverflow: 'ellipsis',
                overflow: !open ? 'hidden' : ''
              }}>
                Женственные балетки базовых цветов подойдут почти под все элементы гардероба. Балетки хорошо
                сочетаются с джинсами для создания более повседневного стиля, так и с юбками и брюками для более
                официальных
                мероприятий. При разработке модели использовалась уникальная технология, которая способствует
                улучшенному
                воздухообмену. Балетки дышат, поэтому ноги не потеют даже в жаркие сезоны. Внут...
              </Typography>
              <Typography sx={{fontWeight: 600, fontSize: '15px', color: 'primary.main', lineHeight: '17px'}}
                          onClick={() => setOpen(prev => !prev)}>Развернуть описание</Typography>
            </Box>
            <Box>
              <SectionTitle title='Характеристики:'/>
              <Stack sx={{maxWidth: '400px', width: '100%'}} direction="column" spacing={1}>
                <Box sx={{display: 'flex', justifyContent: 'space-between'}}>
                  <Typography sx={{fontWeight: 400, fontSize: '16px', color: 'grey.main'}}>Бренд:</Typography>
                  <Typography sx={{fontWeight: 400, fontSize: '16px', color: 'text.primary'}}>Attitude</Typography>
                </Box>
                <Box sx={{display: 'flex', justifyContent: 'space-between'}}>
                  <Typography sx={{fontWeight: 400, fontSize: '16px', color: 'grey.main'}}>Артикул:</Typography>
                  <Typography sx={{fontWeight: 400, fontSize: '16px', color: 'text.primary'}}>FRW65646</Typography>
                </Box>
                <Box sx={{display: 'flex', justifyContent: 'space-between'}}>
                  <Typography sx={{fontWeight: 400, fontSize: '16px', color: 'grey.main'}}>Производитель:</Typography>
                  <Typography sx={{fontWeight: 400, fontSize: '16px', color: 'text.primary'}}>Россия</Typography>
                </Box>
              </Stack>
              <Typography sx={{fontWeight: 600, fontSize: '15px', color: 'primary.main', lineHeight: '17px'}}
                          onClick={() => setOpen(prev => !prev)}>Развернуть все характеристики</Typography>
            </Box>
          </Grid>
          <Grid item sx={12} md={6}>
            ...
            <Grid container>
              {product.related.nodes.map((product, index) => (
                  index < 3
                      ? <Grid item xs={6} md={4}><ProductItem product={product}/></Grid>
                      : ''
              ))}
            </Grid>
          </Grid>
        </Grid>
        <SimilarProducts title='Похожие товары' products={product.related.nodes}/>
      </Box>
  )
}
